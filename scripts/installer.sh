...
# Second token for the dedicated sharpener runner — registration tokens are
# single-use, so each runner needs its own.
RUNNER_TOKEN_SHARPENER=$(gh api -X POST "repos/$SCOUT_OWNER/$SCOUT_NAME/actions/runners/registration-token" --jq .token)
ok
...
RUNNER_TOKEN_SHARPENER=$RUNNER_TOKEN_SHARPENER
EOF
